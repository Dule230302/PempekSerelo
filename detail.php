...
function ubahFormatTanggal($tanggal)
{
	...
	$formatBaru = date('d F Y', $timestamp);

	// Mengubah bulan ke dalam bahasa Indonesia
	$bulanInggris = array(
		'January' => 'Januari',
		'February' => 'Februari',
		'March' => 'Maret',
	...
	// Mengganti bulan dalam format baru dengan bahasa Indonesia
	foreach ($bulanInggris as $inggris => $indonesia) {
		$formatBaru = str_replace($inggris, $indonesia, $formatBaru);
	}
...
<?php
$idproduk = $_GET["id"];
$ambil = $koneksi->query("SELECT produk.*, SUM(stok.stok) AS totalstok
              FROM produk
              JOIN stok ON produk.idproduk = stok.idproduk
		...
		<div class="row g-5">
			...
			<div class="col-lg-6 wow fadeInUp" data-wow-delay="0.5s">
				<div class="h-100">
					...
					for ($i = 1; $i <= 5; $i++) {
						if ($rataulasan >= $i) {
							$kritik .= '<span class="fa fa-star checked" style="color:#ffc700;font-size:12pt"></span>';
						} else {
							$kritik .= '<span class="fa fa-star" style="font-size:12pt"></span>';
						}
					}
					if ($hitungulasan >= 1) {
						echo $kritik;
					...
					<br>
					<br>
					<span style="color: #000;font-size:12pt !important;"><?php echo $detail["deskripsiproduk"]; ?></span>
					<div class="row mt-4">
						<div class="w-100"></div>
						<div class="col-md-12">
							<p style="color: #000;font-size:12pt !important;">Tanggal Produksi : <?php echo ubahFormatTanggal($detail["tgl_produksi"]); ?></p>
							<p style="color: #000;font-size:12pt !important;">Tanggal Expired : <?php echo ubahFormatTanggal($detail["tgl_expired"]); ?></p>
							<p style="color: #000;font-size:12pt !important;">Sisa Produk : <?php echo number_format($detail["totalstok"]); ?></p>
						</div>
					</div>
					<form method="post">
						<div class="form-group">
							<label>Beli Produk</label>
							<input type="number" placeholder="Jumlah" min="1" class="form-control" name="jumlah" max="<?php echo number_format($detail["totalstok"]); ?>" required></input>
							<br>
